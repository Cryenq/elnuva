...
export function hydrateApp(root: HTMLElement, _fixture?: InspectSpatialLayoutData, supplied?: DomainStore): AppView {
  const store=supplied??new DomainStore(); let snapshot:StoreSnapshot|undefined, selected:string|null=null, message="Ready to edit the room.", capability:HTMLElement|undefined, capabilityState:CapabilityState="checking", capabilityMessage="Checking WebMCP availability…";
  ...
  const draw=()=>{if(!snapshot)return; root.replaceChildren();const shell=make("div");shell.className="app-shell";const header=make("header");header.className="site-header";header.append(make("p","Spatial workspace"),make("h1","Elnuva"),make("p","Constraint-aware room planning, with you in control."));shell.append(header);const main=make("main");main.className="workspace";const card=make("section");card.className="card layout-card";card.append(make("h2","Active layout"));
    ...
    const aside=make("aside");aside.className="card capability-card";aside.append(make("h2","Agent capability"));capability=make("p",capabilityMessage);capability.className="capability-status";capability.dataset.state=capabilityState;capability.setAttribute("role","status");aside.append(capability);main.append(aside);shell.append(main);root.append(shell);
  };const unsubscribe=store.subscribe(value=>{snapshot=value;draw()});return Object.freeze({setCapabilityStatus(state,value){capabilityState=state;capabilityMessage=value;if(capability){capability.dataset.state=state;capability.textContent=value}},teardown:unsubscribe});
}
