...
function featureControls(snapshot:StoreSnapshot,store:DomainStore,status:(s:string)=>void):HTMLElement{
  ...
  for(const feature of snapshot.workingState.features){
    ...
    const update=keyed(make("button","Update"),`feature:${feature.id}:update`);update.type="submit";update.disabled=!!snapshot.preview;const remove=keyed(make("button","Delete"),`feature:${feature.id}:delete`);remove.type="button";remove.disabled=!!snapshot.preview;remove.addEventListener("click",()=>{const result=store.deleteFeature(feature.id);status(result.ok?`${feature.id} deleted.`:result.error.message)});
    form.append(title,wallLabel,offsetLabel,update,remove);form.addEventListener("submit",event=>{event.preventDefault();if(!whole(offset.value)){status("Feature offset must be a whole number.");return}const result=store.updateFeature(feature.id,{...feature,wall:wall.value as Wall,offsetMm:Number(offset.value)});status(result.ok?`${feature.id} updated.`:result.error.message)});section.append(form)
  }
...
function addConstraintControls(snapshot:StoreSnapshot,store:DomainStore,status:(s:string)=>void):HTMLElement{
  ...
  form.addEventListener("submit",event=>{event.preventDefault();const id=nextId("constraint",snapshot.workingState.constraints.map(c=>c.constraintId));let constraint:Constraint|null=null;
    ...
    if(!constraint){status("This constraint needs valid, distinct referenced furniture or wall features first.");return}const result=store.addConstraint(constraint);status(result.ok?`${id} added.`:result.error.message)
  });
