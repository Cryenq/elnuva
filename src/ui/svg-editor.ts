...
export function startDrag(event: PointerEvent, itemId: string, node: SVGGElement, snapshot: StoreSnapshot, store: DomainStore, render: () => void, status: (s:string)=>void): void {
  if (event.button !== 0 || !event.isPrimary || snapshot.preview) return;
  const item=snapshot.workingState.furniture.find(x=>x.id===itemId); if(!item||item.locked)return;
  const svg=node.ownerSVGElement; if(!svg)return; let active=true; const original=item; let candidate=item;
  try { node.setPointerCapture(event.pointerId); } catch { return; }
  ...
  const clean=(commit:boolean)=>{if(!active)return;active=false;node.removeEventListener("pointermove",update);node.removeEventListener("pointerup",up);node.removeEventListener("pointercancel",cancel);node.removeEventListener("lostpointercapture",lost);try{if(node.hasPointerCapture(event.pointerId))node.releasePointerCapture(event.pointerId);}catch{} if(!commit){render();return;} if(candidate.xMm===original.xMm&&candidate.yMm===original.yMm){render();return;} if(!placementValid(candidate as Furniture,snapshot.workingState.room,snapshot.workingState.furniture,snapshot.workingState.features)){status("Move rejected: outside bounds, overlap, or radiator keep-out.");render();return;} const result=store.updateFurniturePose(itemId,candidate);status(result.ok?`${itemId} moved to ${candidate.xMm}, ${candidate.yMm} mm.`:result.error.message);render();};
  const up=(e:PointerEvent)=>{if(e.pointerId===event.pointerId)clean(true)};const cancel=(e:PointerEvent)=>{if(e.pointerId===event.pointerId)clean(false)};const lost=(e:PointerEvent)=>{if(e.pointerId===event.pointerId)clean(false)}; node.addEventListener("pointermove",update);node.addEventListener("pointerup",up);node.addEventListener("pointercancel",cancel);node.addEventListener("lostpointercapture",lost);
}
