...
export function reviewPanel(preview: PreviewState | null, callbacks: ReviewCallbacks): HTMLElement {
  ...
  addMetric("Option", validation.optionId, "optionId"); addMetric("Moved", String(validation.movedCount)); addMetric("Rotated", String(validation.rotatedCount)); addMetric("Movement", `${validation.totalMovementMm} mm`); addMetric("Clearance", `${validation.minimumClearanceMm} mm`); addMetric("Required", `${validation.required.satisfied}/${validation.required.total}`, "previewRequired"); addMetric("Preferred", `${validation.preferred.satisfied}/${validation.preferred.total}`, "previewPreferred"); addMetric("Validation", validation.hardValid && validation.stageable ? "Valid · stageable" : "Needs attention"); section.append(metrics);
  ...
  const apply = document.createElement("button"); apply.type = "button"; apply.textContent = "Apply preview"; apply.dataset.previewApply = ""; apply.className = "primary-action"; apply.addEventListener("click", callbacks.apply);
  const discard = document.createElement("button"); discard.type = "button"; discard.textContent = "Discard preview"; discard.dataset.previewDiscard = ""; discard.addEventListener("click", callbacks.discard); actions.append(apply, discard); section.append(actions);
  return section;
}
