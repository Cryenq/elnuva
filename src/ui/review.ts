...
function previewDetails(content: HTMLElement): HTMLDetailsElement {
  const details = document.createElement("details");
  details.dataset.reviewDetails = "";
  details.setAttribute("aria-label", "Preview details");
  const summary = document.createElement("summary");
  summary.textContent = "Preview details";
  summary.dataset.focusKey = "review:details";
  content.className = "review-detail-content";
  content.tabIndex = 0;
  content.dataset.focusKey = "review:content";
  content.setAttribute("role", "region");
  content.setAttribute("aria-label", "Preview details content");
...
function humanFitReview(preview: HumanFitPreview, callbacks: ReviewCallbacks): HTMLElement {
  ...
  const overview = make("div"); overview.className = "preview-overview";
  overview.append(make("h2", "Make it Fit preview — Not applied — Not saved"));
  ...
  content.append(room, make("p", "The solid room is your unchanged working reference. Dashed and translucent shapes show the complete target. Apply once to replace the room and inventory; Save remains a separate action."));
  content.append(make("h3", `${preview.request.additions.length} requested additions`));
  ...
  content.append(additions);
  const counts = preview.assessment;
  content.append(make("p", `Required constraints: ${counts.required.satisfied}/${counts.required.total}. Preferred constraints: ${counts.preferred.satisfied}/${counts.preferred.total}.`));
  ...
  content.append(constraints, make("h3", "Complete target furniture"));
  ...
  actions.append(apply, discard); section.append(actions, previewDetails(content)); return section;
}
...
export function reviewPanel(preview: StoreSnapshot["preview"], callbacks: ReviewCallbacks): HTMLElement {
  ...
  const overview = document.createElement("div"); overview.className = "preview-overview";
  const heading = document.createElement("h2"); heading.textContent = "Agent preview review"; overview.append(heading); section.append(overview);
  ...
  const state = document.createElement("strong"); state.textContent = "Pending review · Not applied · Not saved"; status.append(state); overview.append(status);
  const content = document.createElement("div");
  const instruction = document.createElement("p"); instruction.className = "preview-instruction"; instruction.textContent = "Review the staged ghost, then choose Apply or Discard. Only these buttons can resolve this preview."; content.append(instruction);
  ...
  for (const move of preview.moves) { const item = preview.projectedFurniture.find(candidate => candidate.id === move.itemId); const label = item ? furnitureCatalogById(item.catalogId)?.label ?? move.itemId : move.itemId; const li = document.createElement("li"); li.textContent = `${label} → ${move.pose.xMm}, ${move.pose.yMm} mm · ${move.pose.rotationDeg}°`; moved.append(li); } content.append(moved);
  const issues = document.createElement("p"); issues.className = "preview-issues"; issues.dataset.previewIssues = ""; const issueMessages = validation.issues as readonly { message: string }[]; issues.textContent = issueMessages.length === 0 ? "Issues: none" : `Issues: ${issueMessages.map(issue => issue.message).join("; ")}`; content.append(issues);
  ...
  const discard = document.createElement("button"); discard.type = "button"; discard.textContent = "Discard preview"; discard.dataset.previewDiscard = ""; discard.dataset.focusKey = "preview:discard"; discard.addEventListener("click", callbacks.discard); actions.append(apply, discard); section.append(actions, previewDetails(content));
  return section;
}
