...
export function registerWebMcpTools({ document, inspect, validate, stage }: Options): Promise<WebMcpRegistration> {
  ...
  const attempt = (async (): Promise<WebMcpRegistration> => {
    ...
    if (view && view.self !== view.top) return unavailable;
    if (typeof document.modelContext?.registerTool !== "function") return unavailable;
    if (typeof inspect !== "function" || typeof validate !== "function" || typeof stage !== "function") throw new Error("All WebMCP handlers are required.");
    ...
    const tools: ModelContextTool[] = [
      { name: "inspect_spatial_layout", title: "Inspect spatial layout", description: "Read the active Elnuva room, furniture, constraints, revision, and bounded catalogs.", inputSchema: INSPECT_SPATIAL_LAYOUT_INPUT_SCHEMA, annotations: { readOnlyHint: true, untrustedContentHint: true }, execute: (input, options) => inspect(input, options) },
      { name: "validate_layout_options", title: "Validate layout options", description: "Validate and rank one to three concrete furniture move options against the active Elnuva layout without changing state.", inputSchema: VALIDATE_LAYOUT_OPTIONS_INPUT_SCHEMA, annotations: { readOnlyHint: true, untrustedContentHint: true }, execute: (input, options) => validate(input, options) },
      { name: "stage_layout_preview", title: "Stage layout preview", description: "Stage one validated furniture move option as an ephemeral preview for human Apply or Discard; never apply or save it.", inputSchema: STAGE_LAYOUT_PREVIEW_INPUT_SCHEMA, annotations: { readOnlyHint: false, untrustedContentHint: true }, execute: (input, options) => stage(input, options) },
    ];
