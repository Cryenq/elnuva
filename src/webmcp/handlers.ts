...
export function createStageLayoutPreviewHandler(store: Pick<DomainStore, "stage">): StageLayoutPreviewHandler {
  return async (input, options) => { if (aborted(options)) return failure("CANCELLED"); try { return await store.stage(input as StageRequest, options?.signal); } catch { return failure(aborted(options) ? "CANCELLED" : "INTERNAL_ERROR"); } };
}
